...
function renderCatalog() {
  ...
  products.forEach(p => {
    ...
    // ✅ FIX: specsHTML estaba eliminado, se restaura aquí
...
// ✅ VS movido al centro entre los dos slots
function renderSlots() {
  const container = document.querySelector('.comparator-selector');
...
// ── Orden de especificaciones — sin Año ──
const specOrder = ['Procesador', 'RAM', 'Almacenamiento', 'Tarjeta Gráfica', 'Pantalla', 'Precio'];
 
function compareSpecs(pA, pB) {
  return specOrder.map(spec => {
    // ✅ Ignora cualquier campo "Año" que venga del backend
    const a = spec === 'Año' ? null : pA.specs[spec];
    const b = spec === 'Año' ? null : pB.specs[spec];
